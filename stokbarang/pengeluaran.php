...
<nav class="navbar navbar-expand-lg navbar-custom fixed-top">
    ...
    <div class="container mt-5">
        <h2>Daftar Pengeluaran Barang - User View</h2>
        <div class="actions mb-3">
            <form method="GET" action="pengeluaran.php" class="d-inline">
        ...
        <ul class="pagination">
            ...
            for ($i = 1; $i <= $total_pages; $i++): ?>
                <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                    <a class="page-link" href="pengeluaran.php?page=<?= $i ?>&search=<?= htmlspecialchars($search) ?>"><?= $i ?></a>
                </li>
            <?php endfor; ?>
        </ul>
        </nav>
        <form class="d-flex mb-3" method="GET" action="pengeluaran.php">
